...
5. **凭票撤回即失效**：待确认阶段任一方凭据撤回，整条变更进入终态「已撤回」，
   该方凭据标记已用于撤回。
6. **凭据异常不变更状态**：凭据格式错误、哈希不符一律拒绝；哈希与本变更不符时，
   在同一事务内反查全部变更做三分类——命中另一变更槽位提示「凭据已被另一变更绑定」
   （`CREDENTIAL_BOUND_ELSEWHERE`，不可跨变更复用）、全库未命中提示「凭据无法识别」
   （`CREDENTIAL_UNRECOGNIZED`）、反查查询失败提示「暂时无法验证」
   （`CREDENTIAL_UNAVAILABLE`）。任何拒绝都不写入：合同正文、变更记录、版本号三者
   保持原样，凭据也不被消耗；错误消除后用合法凭据可继续完成确认或撤回。落库失败时
   事务 abort（`PERSISTENCE_FAILED`），同样整体不变。失败结果以原因码返回，
   操作处内联说明具体原因；成功、幂等忽略、拒绝三类结果判别返回、互不混淆。
